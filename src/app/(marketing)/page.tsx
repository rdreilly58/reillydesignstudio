...
import { ArrowRight, Package, Download, Briefcase, Pen } from "lucide-react";
      ...
      <section className="relative overflow-hidden px-4 sm:px-6 lg:px-8 pt-24 pb-32">
        ...
        <div className="max-w-5xl mx-auto text-center">
          ...
          <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight text-white leading-tight">
            Design that <br />
            <span className="text-violet-400">moves people<sup className="text-[0.5em]">™</sup></span>
          </h1>
      ...
      <section className="px-4 sm:px-6 lg:px-8 py-24 bg-zinc-900/50">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-zinc-500 mt-2">Three ways to work together.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {services.map((svc) => (
              <Link
                key={svc.title}
                href={svc.href}
                className={`group relative rounded-2xl p-8 bg-gradient-to-b ${svc.color} border border-zinc-800 hover:border-zinc-600 transition-all`}
              >
                ...
                <div className="mt-6 flex items-center gap-1 text-sm text-violet-400">
          ...
          <div className="divide-y divide-zinc-800">
            {recentPosts.map((post) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="group flex items-center justify-between py-6 hover:bg-zinc-900/50 -mx-4 px-4 rounded-xl transition-colors"
              >
                <div>
                  <span className="text-xs text-violet-400 font-medium">{post.tag}</span>
                  <h3 className="text-white font-medium mt-1 group-hover:text-violet-300 transition-colors">
                    {post.title}
                  </h3>
                </div>
      ...
      <section className="px-4 sm:px-6 lg:px-8 py-24">
        <div className="max-w-4xl mx-auto text-center bg-gradient-to-b from-violet-950/40 to-zinc-900 rounded-3xl border border-violet-900/30 p-16">
          ...
          <p className="text-zinc-400 mt-4 text-lg max-w-xl mx-auto">
            Whether it&apos;s a brand identity, a product, or something entirely new —
            let&apos;s figure it out together.
          </p>
          <Link
            href="/contact"
            className="inline-flex items-center gap-2 mt-8 px-8 py-3.5 rounded-full bg-violet-600 hover:bg-violet-500 text-white font-semibold transition-colors"
          >
